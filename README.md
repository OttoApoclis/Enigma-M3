...
<div class="enigma-case">
    <div class="inner-panel">
        ...
        <div class="plug-area">
            <label style="font-size: 12px; color: #aaa;">STECKERBRETT (接线板): 例如 AB CD EF</label>
            <input type="text" id="plugs" placeholder="输入配对字母..." onkeyup="this.value = this.value.toUpperCase()">
        </div>

        <div class="btn-group">
            <button class="action-btn" onclick="importCipher()">📥 导入/批量</button>
            <button class="action-btn" onclick="copyCipher()">📋 复制密文</button>
            <button class="action-btn" onclick="printTelegram()">📡 发送电报 (PDF)</button>
            <button class="action-btn" onclick="resetAll()" style="color: #ff5252; border-color: #ff5252;">⚠️ 销毁记录</button>
        </div>
    </div>
</div>
...
<div id="print-template">
    ...
    <div class="meta-grid">
        ...
        <div style="grid-column: span 2; border-top: 1px solid #000; margin-top: 5px; padding-top: 5px;">
            <strong>SCHLÜSSEL (Key):</strong> [ NUR FÜR OFFIZIERE / OFFICER EYES ONLY ]
        </div>
    </div>
